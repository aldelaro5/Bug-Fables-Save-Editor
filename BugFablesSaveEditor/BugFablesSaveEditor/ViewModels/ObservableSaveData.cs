...
public class ObservableBfSaveData : ObservableObject
{
  ...
  public ObservableBfCollection<EnemyEncounterSaveData, ObservableEnemyEncounterSaveData> EnemyEncounters { get; }
  public ObservableBfCollection<FlagSaveData, ObservableFlagSaveData> Flags { get; }
  public ObservableBfCollection<FlagstringSaveData, ObservableFlagstringSaveData> Flagstrings { get; }
  ...
  public ObservableBfCollection<PartyMemberSaveData, ObservablePartyMemberSaveData> PartyMembers { get; }
  ...
  public ObservableBfCollection<StatBonusSaveData, ObservableStatsBonusSaveData> StatBonuses { get; }

  public ObservableBfSaveData(BfSaveData saveData)
  {
    _saveData = saveData;
    CrystalBerries = new(_saveData.CrystalBerries, cbs => cbs.Select(x => new ObservableFlagSaveData(x)).ToList());
    EnemyEncounters = new(_saveData.EnemyEncounters, cbs => cbs.Select(x => new ObservableEnemyEncounterSaveData(x)).ToList());
    Flags = new(_saveData.Flags, cbs => cbs.Select(x => new ObservableFlagSaveData(x)).ToList());
    Flagvars = new(_saveData.Flagvars, cbs => cbs.Select(x => new ObservableFlagvarSaveData(x)).ToList());
    Flagstrings = new(_saveData.Flagstrings, cbs => cbs.Select(x => new ObservableFlagstringSaveData(x)).ToList());
    Followers = new(_saveData.Followers, cbs => cbs.Select(x => new ObservableBfResource(x)).ToList());
    Medals = new(_saveData.Medals, cbs => cbs.Select(x => new ObservableMedalOnHandSaveData(x)).ToList());
    PartyMembers = new(_saveData.PartyMembers, cbs => cbs.Select(x => new ObservablePartyMemberSaveData(x)).ToList());
    RegionalFlags = new(_saveData.RegionalFlags, cbs => cbs.Select(x => new ObservableFlagSaveData(x)).ToList());
    SamiraSongs = new(_saveData.SamiraSongs, cbs => cbs.Select(x => new ObservableMusicSaveData(x)).ToList());
    StatBonuses = new(_saveData.StatBonuses, cbs => cbs.Select(x => new ObservableStatsBonusSaveData(x)).ToList());
